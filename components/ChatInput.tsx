...
type ThinkingLevelKey = "thinkingAuto" | "thinkingOff" | "thinkingMinimal" | "thinkingLow" | "thinkingMedium" | "thinkingHigh" | "thinkingXhigh";
type ThinkingLevelDescKey = `${ThinkingLevelKey}Desc`;
function getThinkingLevelKey(level: typeof THINKING_LEVELS[number]): ThinkingLevelKey {
  const map: Record<typeof THINKING_LEVELS[number], ThinkingLevelKey> = {
    auto: "thinkingAuto",
    off: "thinkingOff",
  ...
  return map[level] ?? "thinkingAuto";
}
function getThinkingLevelDescKey(level: typeof THINKING_LEVELS[number]): ThinkingLevelDescKey {
  return `${getThinkingLevelKey(level)}Desc`;
}
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        {retryInfo && (
          <div style={{
            ...
            {t("retrying", { attempt: retryInfo.attempt, maxAttempts: retryInfo.maxAttempts })}{retryInfo.errorMessage && <span style={{ opacity: 0.7, marginLeft: 4 }}>— {retryInfo.errorMessage}</span>}
          </div>
        )}
        {/* Image previews */}
            ...
            {!isStreaming && onThinkingLevelChange && (
              <div ref={thinkingDropdownRef} style={{ position: "relative" }}>
                ...
                >
                  ...
                  <span>{t("thinkingPrefix")}{(() => {
                    const lvl = thinkingLevel ?? "auto";
                    const key = getThinkingLevelKey(lvl);
                    if (lvl === "auto" || !thinkingLevelMap) return t(key);
                    const mapped = thinkingLevelMap[lvl];
                    return mapped != null ? mapped : t(key);
                  })()}</span>
                </button>
                {thinkingDropdownOpen && (
                  <div style={{
                    ...
                    {THINKING_LEVELS.filter((lvl) => {
                      ...
                      const descKey = getThinkingLevelDescKey(lvl);
            ...
            {!isStreaming && onToolPresetChange && (
              <div ref={toolDropdownRef} style={{ position: "relative" }}>
                ...
                >
                  ...
                  <span>{t("toolPrefix")}{(() => {
                    const raw = (Object.entries(TOOL_PRESET_MAP).find(([, v]) => v === (toolPreset ?? "default"))?.[0] ?? "default") as "off" | "default" | "full";
                    const key = raw === "off" ? "toolOff" as const : raw === "default" ? "toolLow" as const : "toolHigh" as const;
                    return t(key);
                  })()}</span>
                </button>
                {toolDropdownOpen && (
                  <div style={{
                    ...
                    {TOOL_PRESETS.map((lvl) => {
                      ...
                      const desc = lvl === "off" ? t("noTools") : lvl === "default" ? t("defaultTools") : t("allTools");
                      return (
                        <button
                          key={lvl}
