...
const TOOL_PRESET_LABEL_KEYS: Record<typeof TOOL_PRESETS[number], "toolOff" | "toolLow" | "toolHigh"> = {
  off: "toolOff",
  default: "toolLow",
  full: "toolHigh",
};
const TOOL_PRESET_DESC_KEYS: Record<typeof TOOL_PRESETS[number], "toolOffDesc" | "toolLowDesc" | "toolHighDesc"> = {
  off: "toolOffDesc",
  default: "toolLowDesc",
  full: "toolHighDesc",
};
...
const THINKING_LEVEL_LABEL_KEYS: Record<typeof THINKING_LEVELS[number], "thinkingAuto" | "thinkingOff" | "thinkingMinimal" | "thinkingLow" | "thinkingMedium" | "thinkingHigh" | "thinkingXhigh"> = {
  auto: "thinkingAuto",
  off: "thinkingOff",
...
const THINKING_LEVEL_DESC_KEYS: Record<typeof THINKING_LEVELS[number], "thinkingAutoDesc" | "thinkingOffDesc" | "thinkingMinimalDesc" | "thinkingLowDesc" | "thinkingMediumDesc" | "thinkingHighDesc" | "thinkingXhighDesc"> = {
  auto: "thinkingAutoDesc",
  off: "thinkingOffDesc",
  minimal: "thinkingMinimalDesc",
  low: "thinkingLowDesc",
  medium: "thinkingMediumDesc",
  high: "thinkingHighDesc",
  xhigh: "thinkingXhighDesc",
};
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const modelOptions: ModelOption[] = (() => {
    ...
    return Object.entries(modelNames ?? {}).map(([modelId, name]) => ({
      provider: model?.provider ?? t("unknown"),
      modelId,
      name,
    }));
  })();
  ...
  const selectedToolPreset = (Object.entries(TOOL_PRESET_MAP).find(([, v]) => v === (toolPreset ?? "default"))?.[0] ?? "default") as typeof TOOL_PRESETS[number];
              ...
              <div ref={thinkingDropdownRef} style={{ position: "relative" }}>
                ...
                >
                  ...
                  <span>{(() => {
                    ...
                    const defaultLabel = t(THINKING_LEVEL_LABEL_KEYS[lvl]);
                    if (lvl === "auto" || !thinkingLevelMap) return defaultLabel;
                    const mapped = thinkingLevelMap[lvl];
                    return mapped != null ? mapped : defaultLabel;
                  })()}</span>
                </button>
                {thinkingDropdownOpen && (
                  <div style={{
                    ...
                    {THINKING_LEVELS.filter((lvl) => {
                      ...
                      const displayLabel = (mappedVal != null && mappedVal !== lvl) ? mappedVal : t(THINKING_LEVEL_LABEL_KEYS[lvl]);
            ...
            {!isStreaming && onToolPresetChange && (
              <div ref={toolDropdownRef} style={{ position: "relative" }}>
                ...
                >
                  ...
                  <span>{t(TOOL_PRESET_LABEL_KEYS[selectedToolPreset])}</span>
                </button>
                {toolDropdownOpen && (
                  <div style={{
                    position: "absolute", bottom: "calc(100% + 6px)", right: 0,
                    ...
                    {TOOL_PRESETS.map((lvl) => {
                      ...
                      const desc = t(TOOL_PRESET_DESC_KEYS[lvl]);
                      return (
                        <button
                          key={lvl}
                        ...
                        >
                          {isActive
                            ? <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><polyline points="1.5 5 4 7.5 8.5 2.5" /></svg>
                            : <span style={{ width: 10, flexShrink: 0 }} />}
                          <span style={{ flex: 1 }}>{t(TOOL_PRESET_LABEL_KEYS[lvl])}</span>
                          <span style={{ fontSize: 11, color: "var(--text-dim)", marginLeft: 8 }}>{desc}</span>
                        </button>
                      );
                    })}
            ...
            {!isStreaming && onCompact && (
              <div style={{ position: "relative" }}>
                ...
                <button
                  ...
                  title={isCompacting ? t("stopCompact") : t("compactContext")}
                >
                  {isCompacting ? (
