...
function formatRelativeTime(dateStr: string, st: ReturnType<typeof useTranslations<"sidebar">>): string {
  ...
  if (mins < 1) return st("justNow");
  if (mins < 60) return st("minutesAgo", { m: mins });
  if (hours < 24) return st("hoursAgo", { h: hours });
  if (days < 7) return st("daysAgo", { d: days });
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button
              ...
              title={selectedCwd ? `New session in ${selectedCwd}` : t("selectProjectFirst")}
              onMouseEnter={(e) => {
                if (!selectedCwd) return;
              ...
              {!customPathOpen ? (
                ...
                <div style={{ padding: "6px 8px", borderTop: recentCwds.length > 0 ? "none" : undefined }}>
                  ...
                  <div style={{ display: "flex", gap: 5, marginTop: 5 }}>
                    ...
                    >
                      {customPathValidating ? t("validating") : t("open")}
                    </button>
                    <button
      ...
      {confirmDelete ? (
        ...
        <>
          <div style={{ flex: 1, minWidth: 0, fontSize: 12, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {st("deleteConfirm")} <span style={{ fontWeight: 600 }}>&ldquo;{title.slice(0, 22)}{title.length > 22 ? "…" : ""}&rdquo;</span>?
          </div>
        ...
        <>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: 2, display: "flex", gap: 8, color: "var(--text-dim)", fontSize: 11 }}>
              <span title={session.modified}>{formatRelativeTime(session.modified, st)}</span>
              <span>{session.messageCount} {st("msgs")}</span>
            </div>
          </div>
          ...
          {hasChildren && (
            <button
              ...
              title={collapsed ? st("expandForks") : st("collapseForks")}
              style={{
                display: "flex", alignItems: "center", justifyContent: "center",
